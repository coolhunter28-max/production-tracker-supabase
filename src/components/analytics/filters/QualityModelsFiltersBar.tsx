...
export function QualityModelsFiltersBar({ styles }: QualityModelsFiltersBarProps) {
  ...
  function updateFilter(value: string) {
    const query = buildQueryString(safeSearchParams, { style: value });
    ...
    <section className="rounded-2xl border bg-card p-4 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div className="space-y-2">
          ...
          <select className="w-full rounded-xl border bg-background px-3 py-2 text-sm" value={current.style} onChange={(e) => updateFilter(e.target.value)}>
