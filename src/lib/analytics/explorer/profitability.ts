...
export type ExplorerProfitabilityByCustomerRow = {
  ranking: number;
  customer: string;
...
type ExplorerProfitabilityByCustomerRpcRow = {
...
export async function getExplorerProfitabilityByCustomer(
  context: AnalysisContext,
  limit = 20,
): Promise<ExplorerProfitabilityByCustomerRow[]> {
  const supabase = await createClient();

  const { data, error } = await supabase.rpc(
    "get_explorer_profitability_by_customer_v1",
    {
      p_seasons:
  ...
  if (error) {
    throw new Error(
      `[getExplorerProfitabilityByCustomer] No se pudo cargar el ranking: ${error.message}`,
    );
  }

  const rows =
    (data ??
      []) as unknown as ExplorerProfitabilityByCustomerRpcRow[];
