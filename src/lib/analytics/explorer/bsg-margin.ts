...
type ExplorerBsgMarginByCustomerRpcRow = {
...
export async function getExplorerBsgMarginByCustomer(
  context: AnalysisContext,
  limit = 20,
): Promise<ExplorerBsgMarginByCustomerRow[]> {
  const supabase = await createClient();

  const { data, error } = await supabase.rpc(
    "get_explorer_bsg_margin_by_customer_v1",
    {
      p_seasons:
  ...
  if (error) {
    throw new Error(
      `[getExplorerBsgMarginByCustomer] No se pudo cargar el ranking: ${error.message}`,
    );
  }

  const rows =
    (data ?? []) as unknown as ExplorerBsgMarginByCustomerRpcRow[];
